...
- **Direct connection (no Tech Provider).** Embedded Signup only onboards other businesses once OY Labs is an approved Tech Provider. Until then — and for any business whose Meta settings OY Labs manages — staff can connect a number from the console workspace page with the WhatsApp Business Account ID, phone number ID and a never-expiring system-user token (generated in that business's Business Settings for the OY Labs Messaging app, with whatsapp_business_management and whatsapp_business_messaging). `connectWithSystemToken` checks the token's validity and scopes, subscribes the account's webhooks, and optionally registers the number with a chosen PIN. Coexistence is not available this way.
