...
- [ ] Webhooks (Pages, Instagram, WhatsApp): run `node --env-file=.env.production scripts/meta-setup.mjs` on the server — no dashboard pasting needed. For reference, WhatsApp subscribes `messages`, `smb_message_echoes`, `smb_app_state_sync`, `history`, `message_template_status_update` and `account_update` on the WhatsApp Business Account object, pointing at the same `/api/meta/webhook` URL and verify token.
