...
function seedData(db: Database.Database) {
  ...
  const items = [
    ...
    },
    {
      ...
      description: 'Custom optimized e-commerce architectures integrating WordPress and Shopify frameworks.',
      longDescription:
...
export function createItem(input: PortfolioItemInput): PortfolioItem {
  ...
  const result = db
    .prepare(
      `INSERT INTO portfolio_items (title, description, long_description, main_category, tags, item_order, cover_image)
       VALUES (@title, @description, @longDescription, @mainCategory, @tags, @order, @coverImage)`
    )
    .run({
      title: input.title,
  ...
  db.prepare(
    `UPDATE portfolio_items
     SET title=@title, description=@description, long_description=@longDescription,
         ...
         cover_image=@coverImage, updated_at=datetime('now')
     WHERE id=@id`
  ).run(merged);
