    ...
    <>
      <motion.header
        ...
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          scrolled ? 'bg-black/60 backdrop-blur-md border-b border-[#222]' : 'bg-transparent'
        }`}
      >
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-0.5 select-none group">
            ...
            <span className="text-[#71717a] font-light text-lg tracking-[0.2em] ml-1.5 group-hover:text-white/60 transition-colors">
              LABS
            </span>
          </Link>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="fixed top-16 left-0 right-0 z-40 bg-[#000]/95 backdrop-blur-md border-b border-[#222] p-6 flex flex-col gap-4 md:hidden"
          >
            {NAV_LINKS.map((link) => (
              <Link
                key={link.href}
