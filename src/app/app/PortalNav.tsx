...
import { Code2, CreditCard, LayoutDashboard, MessagesSquare, Plug, Settings, Users } from 'lucide-react';
...
export default function PortalNav({ isOwner }: { isOwner: boolean }) {
  const pathname = usePathname();
  return (
    <nav className="flex gap-1 overflow-x-auto px-3 pb-3 lg:flex-col lg:pb-0">
      {ITEMS.filter((i) => isOwner || i.href !== '/app/billing').map(({ href, label, icon: Icon, exact }) => {
