...
export default async function PortalOverview({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  const steps = [
    ...
    { done: used > 0, label: 'Connect your Facebook Page and Instagram', href: '/app/connections', icon: Plug, detail: used > 0 ? `${used} connected.` : 'Takes about a minute with Facebook.' },
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <PageHeader eyebrow={ctx.workspace.name} title={`Hello, ${ctx.user.name.split(' ')[0]}`} description="Your Messenger and Instagram messaging at a glance." />
      ...
      <div className="mb-8 grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <Stat label="Pages connected" value={`${used} / ${pageLimit(w)}`} href="/app/connections" />
      ...
      <div className="grid gap-6 lg:grid-cols-5">
        ...
        <Card title="Recent conversations" className="lg:col-span-2" actions={<Link href="/app/inbox" className="text-xs text-ink-dim hover:text-ink">Open inbox</Link>}>
          {conversations.length === 0 ? (
            <p className="text-sm text-ink-dim">No messages yet. Once your Page is connected, new Messenger and Instagram messages appear here.</p>
          ) : (
